	...
	protected void verInicio(HttpServletRequest request, HttpServletResponse response, String path)
		...
		if (path.contains("/validar")) {
			...
			Empresa e = request.getSession().getAttribute("empresa") != null
					...
					: null;
			if (e != null) {
				RangoNumeracion rg= null;
				switch (path) {
				case "ver":
					request.setAttribute("facturas", fDAO.listarPorEmpresa(e.getNit()));
				...
				case "emitir":
					...
					rg= rgDAO.findLast(e.getNit());
					if (cl != null && rg!=null) {
						request.setAttribute("cliente", cl);
						request.setAttribute("rango", rg);
						request.getRequestDispatcher("/Dashboard/EFClienteProducto.jsp").include(request, response);
					} else {
						request.setAttribute("mensaje", "Registre el rango de numeración antes de emitir factura");
				...
				case "rango":
					...
					if(rg==null || rg.getNumeroActual()>=rg.getNumeroHasta()) {
						...
						request.setAttribute("mensaje", "Ya existe un rango de numeración para esta empresa");
						request.getRequestDispatcher("/inicio").forward(request, response);
					}
				...
				request.setAttribute("mensaje", "No tiene registrada la empresa");
				request.getRequestDispatcher("/inicio").forward(request, response);
			}
		}
	}
	...
	protected void agregarRango(HttpServletRequest request, HttpServletResponse response)
		...
		if (rg == null || rg.getNumeroActual() < rg.getNumeroHasta()) {
			try {
				...
				rg= rgDAO.findLastId();
				RangoNumeracion r = new RangoNumeracion(rg!=null?rg.getIdNumeracion()+1:0,fechaResolucion, actual, desde, hasta, numeroResolucion,
							prefijo);
