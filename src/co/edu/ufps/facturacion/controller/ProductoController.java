	...
	protected void verInicio(HttpServletRequest request, HttpServletResponse response, String path)
		...
		if (path.contains("/validar")) {
			...
			if (e != null) {
				switch (path) {
				case "ver":
					request.setAttribute("productos", pDAO.listarPorEmpresa(e.getNit()));
				...
				case "editar":
					...
					if (p != null && p.getEmpresa().getNit()==e.getNit()) {
						request.setAttribute("producto", p);
						request.getRequestDispatcher("/Dashboard/editarProducto.jsp").include(request, response);
					} else {
						request.setAttribute("mensaje", "No se encuentra registrado el producto para esta empresa");
