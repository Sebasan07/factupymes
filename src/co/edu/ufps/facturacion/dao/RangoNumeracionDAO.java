...
public class RangoNumeracionDAO extends Conexion<RangoNumeracion> implements GenericDAO<RangoNumeracion>{
	...
	public RangoNumeracion findLast(int nit) {
		...
		try {
				query = getEm().createNamedQuery(RangoNumeracion.class.getSimpleName() + ".findByEmpresa", RangoNumeracion.class).setParameter("nit", nit);
	...
	public RangoNumeracion findLastId() {
		Query query = null;
		RangoNumeracion r=null;
		try {
